package com.king.base.utils.type;
...
public class ParameterizedTypeImpl implements ParameterizedType {
    ...
    private void checkArgs() {
        if (raw == null) {
            throw new RuntimeException("raw class can't be null");
        }
        TypeVariable[] typeParameters = raw.getTypeParameters();
        if (args.length != 0 && typeParameters.length != args.length) {
            throw new RuntimeException(raw.getName() + " expect " + typeParameters.length + " arg(s), got " + args.length);
        }
    }
